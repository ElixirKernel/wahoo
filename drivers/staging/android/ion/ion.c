...
static void ion_handle_free_work(struct work_struct *work)
{
	struct ion_handle *handle = container_of(work, typeof(*handle), free);
	struct ion_buffer *buffer = handle->buffer;
	struct ion_client *client = handle->client;

	ion_buffer_put(buffer);

	spin_lock(&client->handle_lock);
	idr_remove(&client->handles, handle->id);
	spin_unlock(&client->handle_lock);
					    ...
					    struct ion_buffer *buffer)
{
	...
	*handle = (typeof(*handle)){
		.buffer = buffer,
		.client = client,
		.free = __WORK_INITIALIZER(handle->free, ion_handle_free_work),
	};

	idr_preload(GFP_KERNEL);
	spin_lock(&client->handle_lock);
	handle->id = idr_alloc(&client->handles, handle, 1, 0, GFP_ATOMIC);
	spin_unlock(&client->handle_lock);
	idr_preload_end();
...
struct ion_buffer *__ion_import_dma_buf(struct ion_client *client, int fd)
{
	struct ion_buffer *buffer;
	struct dma_buf *dmabuf;
...
static long ion_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ION_IOC_FREE:
		...
		queue_work(client->wq, &handle->free);
		return 0;
	case ION_IOC_SHARE:
	case ION_IOC_MAP:
		handle = ion_handle_find_by_id(client, data.handle.handle);
		...
		break;
	case ION_IOC_IMPORT:
		buffer = __ion_import_dma_buf(client, data.fd.fd);
		if (IS_ERR(buffer))
			return PTR_ERR(buffer);
...
static int ion_open(struct inode *inode, struct file *file)
{
	...
	*client = (typeof(*client)){
		.idev = idev,
		.handles = IDR_INIT(client->handles),
		.handle_lock = __SPIN_LOCK_UNLOCKED(client->handle_lock)
	};

	client->wq = alloc_workqueue("ion", WQ_UNBOUND, WQ_UNBOUND_MAX_ACTIVE);
	if (!client->wq) {
		kfree(client);
		return -ENOMEM;
	}
